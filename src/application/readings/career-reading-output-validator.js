...
function createInterpretationInput(context) {
  ...
  return immutableCopy({ schemaVersion: CAREER_READING_INTERPRETATION_SCHEMA_VERSION, calibrationLevel: context.calibrationLevel, eventCount: (context.eventReferences || []).length, eventReferences: (context.eventReferences || []).map((item) => ({ eventId: item.eventId, eventDate: item.eventDate })), historicalEvidence: [...maps.historical.values()].map((item) => ({ evidenceId: item.evidenceId, matchedEventCount: item.matchedEventCount, eligibleEventCount: item.eligibleEventCount, recurrenceRate: item.recurrenceRate })), futureOccurrences: [...maps.future.values()].map((item) => ({ evidenceId: item.evidenceId, occurrenceType: item.occurrenceType, from: item.from, to: item.to, instant: item.instant })), composites: [...maps.composites.values()].map((item) => ({ evidenceId: item.evidenceId, from: item.from, to: item.to })), allowedFactors: [...maps.factors].sort(), hasProvisionalEvidence: Boolean(context.calculationBasis && context.calculationBasis.hasProvisionalEvidence) });
}
