...
class CareerReadingInterpreter {
  ...
  async interpret({ principal, birthProfileId } = {}) { return this.interpretContext(await this.contextBuilder.build({ principal, birthProfileId })); }
  async interpretContext(context) { const interpretationInput = createInterpretationInput(context); let candidate; try { candidate = await this.generator.generate({ interpretationInput }); } catch (error) { if (error && error.code === 'READING_GENERATION_FAILED') throw error; fail('READING_GENERATION_FAILED'); } try { return this.validator.validate({ context, candidate }); } catch (error) { if (error && error.code === 'READING_GENERATION_INVALID_OUTPUT') throw error; fail('READING_GENERATION_INVALID_OUTPUT'); } }
}
