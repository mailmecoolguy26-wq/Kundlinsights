...
function dtoReadingDetail(reading) { return { ...dtoReadingSummary(reading), content: reading.content === undefined ? null : reading.content, ...(reading.calibratedContent === undefined ? {} : { calibratedContent: reading.calibratedContent }), ...(reading.insights === undefined ? {} : { insights: reading.insights }) }; }
function createApi({ authVerifier, userResolver, birthProfileService, careerEventService = null, careerEventAstrologyService = null, natalSummaryService = null, divisionalChartService = null, vimshottariService = null, transitSnapshotService = null, ashtakavargaService = null, secureReadingService, purchaseService = null, razorpayPaymentService = null, appleNotificationService = null, googleRtdnService = null, placeResolutionService = null, entitlementService, requestIdGenerator = crypto.randomUUID, corsAllowlist = [], isReady = () => true, logger = false, bodyLimit = 16 * 1024 } = {}) {
  ...
  app.addContentTypeParser('application/json', { parseAs: 'buffer' }, (request, body, done) => {
    if (request.url.split('?', 1)[0] === '/v1/payments/razorpay/webhook') return done(null, body);
  ...
  app.addHook('preHandler', async (request) => { const pathname = request.url.split('?', 1)[0]; if (pathname === '/health' || pathname === '/ready' || pathname === '/v1/webhooks/apple' || pathname === '/v1/webhooks/google' || pathname === '/v1/payments/razorpay/webhook') return; request.principal = await authVerifier.verifyRequest(request); });
  ...
  if (razorpayPaymentService) {
    app.post('/v1/payments/razorpay/orders', async (request, reply) => { const body = request.body || {}; const order = await razorpayPaymentService.createOrder({ principal: request.principal, logicalSku: body.logicalSku, birthProfileId: body.birthProfileId || null }); reply.code(201); return { order, requestId: request.id }; });
    app.post('/v1/payments/razorpay/verify', async (request) => { const body = request.body || {}; return { ...await razorpayPaymentService.verifyAndFinalize({ principal: request.principal, orderId: body.razorpayOrderId, paymentId: body.razorpayPaymentId, signature: body.razorpaySignature }), requestId: request.id }; });
    app.get('/v1/payments/razorpay/orders/:id', async (request) => ({ order: await razorpayPaymentService.getOrderStatus({ principal: request.principal, orderId: id(request.params.id, 'RAZORPAY_ORDER_ID') }), requestId: request.id }));
    app.get('/v1/payments/razorpay/unresolved-order', async (request) => ({ order: await razorpayPaymentService.getLatestUnresolvedForProfile({ principal: request.principal, birthProfileId: id(request.query && request.query.birthProfileId, 'BIRTH_PROFILE_ID') }), requestId: request.id }));
    app.post('/v1/payments/razorpay/webhook', async (request) => razorpayPaymentService.handleWebhook({ rawBody: request.body, signature: request.headers['x-razorpay-signature'] }));
  }
