...
function googlePlay(env) { const productIds = [env.GOOGLE_CAREER_PREMIUM_ANNUAL_PRODUCT_ID, env.GOOGLE_CAREER_PROFILE_UNLOCK_PRODUCT_ID]; const values = [env.GOOGLE_PACKAGE_NAME, env.GOOGLE_PLAY_SERVICE_ACCOUNT_JSON]; if (values.concat(productIds).every((value) => value === undefined || value === '')) return null; if (!values.every(text) || !productIds.some(text) || !productIds.every((value) => value === undefined || value === '' || text(value))) invalid(); try { const rtdnValues = [env.GOOGLE_RTDN_AUDIENCE, env.GOOGLE_RTDN_ALLOWED_SERVICE_ACCOUNT_EMAIL]; if (!rtdnValues.every((value) => value === undefined || value === '') && (!rtdnValues.every(text) || !text(env.GOOGLE_CAREER_PREMIUM_ANNUAL_PRODUCT_ID))) invalid(); return Object.freeze({ packageName: env.GOOGLE_PACKAGE_NAME, careerPremiumAnnualProductId: text(env.GOOGLE_CAREER_PREMIUM_ANNUAL_PRODUCT_ID) ? env.GOOGLE_CAREER_PREMIUM_ANNUAL_PRODUCT_ID : null, careerProfileUnlockProductId: text(env.GOOGLE_CAREER_PROFILE_UNLOCK_PRODUCT_ID) ? env.GOOGLE_CAREER_PROFILE_UNLOCK_PRODUCT_ID : null, serviceAccount: JSON.parse(env.GOOGLE_PLAY_SERVICE_ACCOUNT_JSON), rtdn: rtdnValues.every(text) ? Object.freeze({ audience: env.GOOGLE_RTDN_AUDIENCE, allowedServiceAccountEmail: env.GOOGLE_RTDN_ALLOWED_SERVICE_ACCOUNT_EMAIL }) : null }); } catch { invalid(); } }
function razorpay(env) { const values = [env.RAZORPAY_KEY_ID, env.RAZORPAY_KEY_SECRET, env.RAZORPAY_WEBHOOK_SECRET, env.RAZORPAY_PRODUCT_CATALOG_JSON]; if (values.every((value) => value === undefined || value === '')) return null; if (!values.every(text)) invalid(); try { const products = JSON.parse(env.RAZORPAY_PRODUCT_CATALOG_JSON); if (!Array.isArray(products)) invalid(); return Object.freeze({ keyId: env.RAZORPAY_KEY_ID, keySecret: env.RAZORPAY_KEY_SECRET, webhookSecret: env.RAZORPAY_WEBHOOK_SECRET, products }); } catch { invalid(); } }
...
function loadProductionConfig(env = process.env) {
  ...
  return Object.freeze({
    ...
    auth: Object.freeze({ issuer, jwksUri, audience: env.SUPABASE_AUTH_AUDIENCE, allowedAlgorithms: algorithms(env.SUPABASE_AUTH_ALLOWED_ALGORITHMS) }), apple: Object.freeze({ bundleId: text(env.APPLE_BUNDLE_ID) ? env.APPLE_BUNDLE_ID : null, careerPremiumAnnualProductId: text(env.APPLE_CAREER_PREMIUM_ANNUAL_PRODUCT_ID) ? env.APPLE_CAREER_PREMIUM_ANNUAL_PRODUCT_ID : null }),
