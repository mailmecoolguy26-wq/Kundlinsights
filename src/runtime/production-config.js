...
function googlePlay(env) { const values = [env.GOOGLE_PACKAGE_NAME, env.GOOGLE_CAREER_PREMIUM_ANNUAL_PRODUCT_ID, env.GOOGLE_PLAY_SERVICE_ACCOUNT_JSON]; if (values.every((value) => value === undefined || value === '')) return null; if (!values.every(text)) invalid(); try { const rtdnValues = [env.GOOGLE_RTDN_AUDIENCE, env.GOOGLE_RTDN_ALLOWED_SERVICE_ACCOUNT_EMAIL]; if (!rtdnValues.every((value) => value === undefined || value === '') && !rtdnValues.every(text)) invalid(); return Object.freeze({ packageName: env.GOOGLE_PACKAGE_NAME, careerPremiumAnnualProductId: env.GOOGLE_CAREER_PREMIUM_ANNUAL_PRODUCT_ID, serviceAccount: JSON.parse(env.GOOGLE_PLAY_SERVICE_ACCOUNT_JSON), rtdn: rtdnValues.every(text) ? Object.freeze({ audience: env.GOOGLE_RTDN_AUDIENCE, allowedServiceAccountEmail: env.GOOGLE_RTDN_ALLOWED_SERVICE_ACCOUNT_EMAIL }) : null }); } catch { invalid(); } }
...
function loadProductionConfig(env = process.env) {
  ...
  return Object.freeze({
    ...
    google: Object.freeze({ mapsApiKey: text(env.GOOGLE_MAPS_API_KEY) ? env.GOOGLE_MAPS_API_KEY : invalid(), timeoutMilliseconds: integer(env.GOOGLE_GEOCODING_TIMEOUT_MS, 5000, 100, 15000), play: googlePlay(env) }),
