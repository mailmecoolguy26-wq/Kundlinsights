...
function placeResolver(env) { const values = [env.GOOGLE_MAPS_API_KEY, env.TIMEZONE_RUNTIME_MANIFEST_PATH, env.TIMEZONE_RUNTIME_BINARY_PATH]; if (values.every((value) => value === undefined || value === '')) return null; if (!values.every(text)) invalid(); return Object.freeze({ googleMapsApiKey: env.GOOGLE_MAPS_API_KEY, timeoutMilliseconds: integer(env.GOOGLE_GEOCODING_TIMEOUT_MS, 5000, 100, 15000), manifestPath: env.TIMEZONE_RUNTIME_MANIFEST_PATH, binaryPath: env.TIMEZONE_RUNTIME_BINARY_PATH }); }
function googlePlay(env) { const values = [env.GOOGLE_PACKAGE_NAME, env.GOOGLE_CAREER_PREMIUM_ANNUAL_PRODUCT_ID, env.GOOGLE_PLAY_SERVICE_ACCOUNT_JSON]; if (values.every((value) => value === undefined || value === '')) return null; if (!values.every(text)) invalid(); try { return Object.freeze({ packageName: env.GOOGLE_PACKAGE_NAME, careerPremiumAnnualProductId: env.GOOGLE_CAREER_PREMIUM_ANNUAL_PRODUCT_ID, serviceAccount: JSON.parse(env.GOOGLE_PLAY_SERVICE_ACCOUNT_JSON) }); } catch { invalid(); } }
...
function loadDevelopmentConfig(env = process.env) {
  ...
  return Object.freeze({ host: text(env.DEV_HOST) ? env.DEV_HOST : '0.0.0.0', port: integer(env.DEV_PORT, 3000, 1, 65535), databaseUrl: databaseUrl(env.DEV_DATABASE_URL), placeResolver: placeResolver(env), googlePlay: googlePlay(env), apple: Object.freeze({ bundleId: text(env.APPLE_BUNDLE_ID) ? env.APPLE_BUNDLE_ID : null, careerPremiumAnnualProductId: text(env.APPLE_CAREER_PREMIUM_ANNUAL_PRODUCT_ID) ? env.APPLE_CAREER_PREMIUM_ANNUAL_PRODUCT_ID : null }), auth: Object.freeze({ issuer: httpsUrl(env.DEV_SUPABASE_AUTH_ISSUER), jwksUri: httpsUrl(env.DEV_SUPABASE_AUTH_JWKS_URL), audience: text(env.DEV_SUPABASE_AUTH_AUDIENCE) ? env.DEV_SUPABASE_AUTH_AUDIENCE : invalid(), allowedAlgorithms: Object.freeze([...new Set(algorithms)]) }), localKms: Object.freeze({ key: localKey(env.DEV_LOCAL_KMS_KEY_BASE64), keyVersion: text(env.DEV_LOCAL_KMS_KEY_VERSION) ? env.DEV_LOCAL_KMS_KEY_VERSION : 'development-local-kek-v1' }), corsOrigins: Object.freeze([]), bodyLimitBytes: integer(env.DEV_REQUEST_BODY_LIMIT_BYTES, 16384, 1024, 16384) });
}
