...
const { DevelopmentCareerEntitlementFixture } = require('./development-career-entitlement-fixture');
const { createDevelopmentPlaceResolver } = require('./create-development-place-resolver');
...
function createDevelopmentRuntime({ env = process.env, astronomicalEngine, canonicalSiderealSunSampler, idGenerator = crypto.randomUUID, clock = () => new Date().toISOString(), dependencies = {} } = {}) {
  ...
  const PoolClass = dependencies.Pool || Pool, KmsClass = dependencies.DevelopmentLocalKms || DevelopmentLocalKms, authFactory = dependencies.createSupabaseAuthVerifier || createSupabaseAuthVerifier, compositionFactory = dependencies.createApiComposition || createApiComposition, placeResolverFactory = dependencies.createDevelopmentPlaceResolver || createDevelopmentPlaceResolver;
  ...
  const transactionDiagnosticObserver = (value) => { latestAuthenticatedDbDiagnostic = value; console.error('AUTHENTICATED_DB_DIAGNOSTIC', JSON.stringify(value)); };
  const developmentPlaceResolver = placeResolverFactory({ config: config.placeResolver });
  const composition = compositionFactory({ db, authVerifier, kms, astronomicalEngine, canonicalSiderealSunSampler, placeResolver: developmentPlaceResolver && developmentPlaceResolver.resolver, google: config.googlePlay, razorpay: config.razorpay, careerChat: config.careerChat, idGenerator, clock, corsAllowlist: config.corsOrigins, isReady: () => ready, logger: false, bodyLimit: config.bodyLimitBytes, transactionDiagnosticObserver }); const api = composition.api;
  ...
  function shutdown() { if (shuttingDown) return shuttingDown; shuttingDown = (async () => { ready = false; await Promise.resolve(api.close()); if (developmentPlaceResolver && typeof developmentPlaceResolver.close === 'function') developmentPlaceResolver.close(); if (typeof db.end === 'function') await db.end(); })(); return shuttingDown; }
