...
function createDevelopmentRuntime({ env = process.env, astronomicalEngine, canonicalSiderealSunSampler, idGenerator = crypto.randomUUID, clock = () => new Date().toISOString(), dependencies = {} } = {}) {
  ...
  const composition = compositionFactory({ db, authVerifier, kms, astronomicalEngine, canonicalSiderealSunSampler, placeResolver: developmentPlaceResolver && developmentPlaceResolver.resolver, idGenerator, clock, corsAllowlist: config.corsOrigins, isReady: () => ready, logger: false, bodyLimit: config.bodyLimitBytes, transactionDiagnosticObserver }); const api = composition.api;
