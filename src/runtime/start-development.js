...
async function startDevelopment({ env = process.env, dependencies = {} } = {}) {
  ...
  const runtime = (dependencies.createDevelopmentRuntime || createDevelopmentRuntime)({ env, astronomicalEngine: astrology.astronomicalEngine, canonicalSiderealSunSampler: astrology.canonicalSiderealSunSampler });
  runtime.installSignalHandlers();
  await runtime.start();
  return runtime;
}

if (require.main === module) startDevelopment().catch(() => { process.exitCode = 1; });
