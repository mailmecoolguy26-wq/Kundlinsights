...
const { AppleRootCertificateProvider } = require('../payment/apple/apple-root-certificate-provider');
...
function createProductionRuntime({ env = process.env, astronomicalEngine, canonicalSiderealSunSampler, idGenerator = crypto.randomUUID, clock = () => new Date().toISOString(), dependencies = {} } = {}) {
  ...
  const apple = config.apple && { ...config.apple, rootCertificateProvider: new AppleRootCertificateProvider({ paths: config.apple.rootCertificatePaths }) };
  const composition = compositionFactory({ db, authVerifier, kms, astronomicalEngine, canonicalSiderealSunSampler, placeResolver, openai: config.openai, careerChat: config.careerChat, apple, google: config.google.play, razorpay: config.razorpay, idGenerator, clock, corsAllowlist: config.corsOrigins, isReady: () => ready, logger: productionLogger(config.logLevel), bodyLimit: config.bodyLimitBytes });
