...
function productionLogger(level) { return { level, serializers: { req: requestLogValue }, redact: { paths: ['req.headers.authorization', 'req.headers.cookie', 'request.headers.authorization', 'request.headers.cookie', 'DATABASE_URL', 'AWS_ACCESS_KEY_ID', 'AWS_SECRET_ACCESS_KEY', 'GOOGLE_MAPS_API_KEY'], remove: true } }; }
...
function createProductionRuntime({ env = process.env, astronomicalEngine, canonicalSiderealSunSampler, idGenerator = crypto.randomUUID, clock = () => new Date().toISOString(), dependencies = {} } = {}) {
  ...
  const composition = compositionFactory({ db, authVerifier, kms, astronomicalEngine, canonicalSiderealSunSampler, placeResolver, idGenerator, clock, corsAllowlist: config.corsOrigins, isReady: () => ready, logger: productionLogger(config.logLevel), bodyLimit: config.bodyLimitBytes });
