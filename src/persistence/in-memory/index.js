...
class InMemoryPaymentRepository {
  ...
  insertPaymentTransaction(input = {}) { const id = requiredString(input.id, 'INVALID_PAYMENT_TRANSACTION_ID'); const provider = requiredString(input.provider, 'INVALID_PAYMENT_PROVIDER'); const providerTransactionId = requiredString(input.providerTransactionId, 'INVALID_PROVIDER_TRANSACTION_ID'); const key = `${provider}\u0000${providerTransactionId}`; if (this.payments.has(id)) fail('DUPLICATE_PAYMENT_TRANSACTION_ID'); if (this.byProviderTransaction.has(key)) fail('DUPLICATE_PROVIDER_TRANSACTION'); if (!Number.isFinite(input.amount) || input.amount < 0 || typeof input.currency !== 'string' || !/^[A-Z]{3}$/.test(input.currency)) fail('INVALID_PAYMENT_AMOUNT'); const item = immutableCopy({ id, userId: requiredString(input.userId, 'INVALID_USER_ID'), provider, providerTransactionId, status: status(input.status || 'created'), amount: input.amount, currency: input.currency, createdAt: canonicalTime(input.createdAt), updatedAt: canonicalTime(input.updatedAt || input.createdAt) }); this.payments.set(id, item); this.byProviderTransaction.set(key, id); return copyResult(item); }
