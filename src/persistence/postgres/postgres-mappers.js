...
function birthProfileFromRow(row, codec) {
  const birthData = codec.decodeBirthData({ userId: row.user_id, profileId: row.id, ciphertext: row.birth_payload_ciphertext, encryptionVersion: row.birth_payload_encryption_version, keyVersion: row.birth_payload_key_version, algorithm: row.birth_payload_algorithm, nonce: row.birth_payload_nonce });
...
function encodedBirthPayload(codec, { userId, profileId, birthData }) {
  const value = codec.encodeBirthData({ userId, profileId, birthData });
...
function encodedReadingPayload(codec, { userId, record }) {
  const value = codec.encodeRecord({ userId, record });
  if (!value || typeof value !== 'object') { const error = new RangeError('INVALID_CODEC_PAYLOAD'); error.code = 'INVALID_CODEC_PAYLOAD'; throw error; }
  const out = {
