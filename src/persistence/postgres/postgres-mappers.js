...
async function birthProfileFromRow(row, codec) {
...
function providerPaymentOrderFromRow(row) { return immutableCopy({ id: row.id, userId: row.user_id, birthProfileId: row.birth_profile_id, provider: row.provider, logicalSku: row.logical_sku, providerOrderId: row.provider_order_id, providerPaymentId: row.provider_payment_id, amountMinor: Number(row.amount_minor), currency: row.currency.trim(), status: row.status, purchaseRecordId: row.purchase_record_id, createdAt: dbTime(row.created_at), updatedAt: dbTime(row.updated_at) }); }
function purchaseFromRow(row) { return immutableCopy({ id: row.id, userId: row.user_id, provider: row.provider, environment: row.environment, productId: row.product_id, providerTransactionId: row.provider_transaction_id, originalTransactionId: row.original_transaction_id, status: row.status, purchasedAt: dbTime(row.purchased_at), validFrom: nullableTime(row.valid_from), validUntil: nullableTime(row.valid_until), verifiedAt: dbTime(row.verified_at), paymentTransactionId: row.payment_transaction_id, createdAt: dbTime(row.created_at), updatedAt: dbTime(row.updated_at) }); }
function subscriptionFromRow(row) { return immutableCopy({ id: row.id, userId: row.user_id, provider: row.provider, environment: row.environment, productId: row.product_id, originalTransactionId: row.original_transaction_id, status: row.status, validFrom: dbTime(row.valid_from), validUntil: dbTime(row.valid_until), autoRenewEnabled: row.auto_renew_enabled, graceUntil: nullableTime(row.grace_until), canceledAt: nullableTime(row.canceled_at), revokedAt: nullableTime(row.revoked_at), refundedAt: nullableTime(row.refunded_at), latestPurchaseRecordId: row.latest_purchase_record_id, providerEventTime: nullableTime(row.provider_event_time), providerEventVersion: row.provider_event_version, createdAt: dbTime(row.created_at), updatedAt: dbTime(row.updated_at) }); }
function paymentEventFromRow(row) { return immutableCopy({ id: row.id, provider: row.provider, environment: row.environment, providerEventId: row.provider_event_id, eventType: row.event_type, providerEventTime: nullableTime(row.provider_event_time), purchaseRecordId: row.purchase_record_id, subscriptionRecordId: row.subscription_record_id, receivedAt: dbTime(row.received_at), processedAt: nullableTime(row.processed_at), processingStatus: row.processing_status, failureCode: row.failure_code, payloadDigest: row.payload_digest, createdAt: dbTime(row.created_at), updatedAt: dbTime(row.created_at) }); }
function profileEntitlementFromRow(row) { return immutableCopy({ id: row.id, userId: row.user_id, birthProfileId: row.birth_profile_id, logicalSku: row.logical_sku, purchaseRecordId: row.purchase_record_id, unlockedAt: dbTime(row.unlocked_at), createdAt: dbTime(row.created_at), updatedAt: dbTime(row.updated_at) }); }
...
function careerEventObservationFromRow(row) { return immutableCopy({ id: row.id, userId: row.user_id, birthProfileId: row.birth_profile_id, careerEventId: row.career_event_id, observationType: row.observation_type, eventDatePrecision: row.event_date_precision, eventYear: Number(row.event_year), eventMonth: row.event_month === null ? null : Number(row.event_month), eventDay: row.event_day === null ? null : Number(row.event_day), createdAt: dbTime(row.created_at), updatedAt: dbTime(row.updated_at) }); }
...
async function encodedReadingPayload(codec, { userId, record }) {
  const value = await codec.encodeRecord({ userId, record });
  if (!value || typeof value !== 'object') { const error = new RangeError('INVALID_CODEC_PAYLOAD'); error.code = 'INVALID_CODEC_PAYLOAD'; throw error; }
  const out = {
...
module.exports = { dbTime, nullableTime, nullableString, toBuffer, profileFingerprint, userFromRow, birthProfileFromRow, encryptedBirthProfileFromRow, paymentFromRow, providerPaymentOrderFromRow, purchaseFromRow, subscriptionFromRow, paymentEventFromRow, profileEntitlementFromRow, entitlementFromRow, careerEventFromRow, careerEventObservationFromRow, readingFromRow, encryptedReadingFromRow, encodedBirthPayload, encodedReadingPayload, freeze };
