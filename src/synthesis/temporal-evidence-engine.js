...
function buildTemporalEvidence(input = {}) {
  ...
  if (input.transitEvents === undefined) missing('transitEvents');
    ...
    events.forEach((event) => {
      ...
      const suppliedStart = event.startInstant === undefined ? null : utc(event.startInstant, 'transit event startInstant'); const suppliedEnd = event.endInstant === undefined ? null : utc(event.endInstant, 'transit event endInstant');
      if ((suppliedStart === null) !== (suppliedEnd === null) || suppliedStart && Date.parse(suppliedStart) >= Date.parse(suppliedEnd)) throw new RangeError('Transit event intervals must provide valid startInstant and endInstant with start before end.');
      const temporal = node(`event:${identity}`, { kind: 'FACT', subject: subject('TRANSIT_EVENT', event.body || event.transitBody || event.eventType), sourceLayer: '10', sourceRulesetId: input.transitEvents.rulesetId || 'transit-event-scanner-v1', sourceStrength: 'ENGINE_CONVENTION', temporalContext: suppliedStart ? { id: eventInstant, type: 'INTERVAL', startInstant: suppliedStart, endInstant: suppliedEnd } : { id: eventInstant, type: 'POINT', instant: eventInstant }, fact: { eventType: event.eventType, instant: eventInstant, ...(suppliedStart ? { startInstant: suppliedStart, endInstant: suppliedEnd } : {}), body: event.body || event.transitBody || null, natalBody: event.natalBody || null, targetHouseNumber: event.targetHouseNumber ?? event.toHouseNumber ?? null, sourceEventIdentity: identity, upstreamEvidence: event, upstreamProvenance: input.transitEvents.provenance || null } });
