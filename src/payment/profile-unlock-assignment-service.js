...
const RAZORPAY_CAREER_PREMIUM_SKU = 'career_premium_annual';
...
class ProfileUnlockAssignmentService {
  ...
  async assignInTransaction({ tx, userId, birthProfileId, purchaseRecordId, purchaseProductId = SKU } = {}) {
    ...
    purchaseProductId = requiredString(purchaseProductId, 'INVALID_PRODUCT_ID');
    if (![SKU, RAZORPAY_CAREER_PREMIUM_SKU].includes(purchaseProductId)) fail('PURCHASE_PRODUCT_UNSUPPORTED');
    const purchase = await tx.purchases.findById(purchaseRecordId); if (!purchase) fail('PURCHASE_NOT_FOUND'); if (purchase.status !== 'VERIFIED') fail('PURCHASE_NOT_VERIFIED'); if (purchase.productId !== purchaseProductId) fail('PURCHASE_PRODUCT_UNSUPPORTED'); if (purchaseProductId === RAZORPAY_CAREER_PREMIUM_SKU && purchase.provider !== 'RAZORPAY') fail('PURCHASE_PROVIDER_UNSUPPORTED'); if (purchase.userId !== userId) fail('PURCHASE_OWNERSHIP_CONFLICT');
