...
async function safeAsyncCrypto(operation, fallback) { try { return await operation(); } catch (error) { if (error && error.code && /^[A-Z_]+$/.test(error.code)) throw error; fail(fallback); } }

module.exports = { cryptoError, fail, safeCrypto, safeAsyncCrypto };
