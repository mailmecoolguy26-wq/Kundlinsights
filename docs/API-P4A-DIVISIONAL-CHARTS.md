...
Each safe DTO contains the chart type, a separate Ascendant, ordered `houses`, and the nine Grahas. A Graha exposes its authoritative resulting-Rashi sign, `degreeWithinSign`, and house. That degree is the TaraVerse Varga engine coordinate within the derived resulting Rashi; it is not a natal longitude. Nakshatra/Pada, retrograde/motion, natal longitudes, raw birth data, encryption/KMS data, provider internals, IDs other than the requested profile ID, and interpretation are deliberately omitted.
