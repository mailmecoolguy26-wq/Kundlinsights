...
Sade Sati is an engine-level sign-phase detector: Saturn at H12/H1/H2 from natal Moon is respectively `rising`, `peak`, or `setting`; all other positions are `none`. The labels are TaraVerse conventions, not claimed classical terminology. Degree, Nakshatra, Pada, Vedha, Dhaiya, Ashtama Shani, and interpretation are deferred.
