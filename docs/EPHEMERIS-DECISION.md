...
This repository must not add the AGPL Swiss Ephemeris package as an unreviewed production dependency. The future `SwissEphemerisProvider` implementation is therefore a controlled integration task, gated on recording the executed license and the exact Swiss Ephemeris/data release in the dependency inventory.
