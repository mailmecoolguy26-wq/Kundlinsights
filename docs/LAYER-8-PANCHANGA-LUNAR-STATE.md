...
Exact `0°` is represented as `newMoon`; exact `180°` as `fullMoon`; intervening states are `waxing` and `waning`. These exact state labels and all `[start,end)` interval handling are TaraVerse engine conventions. `360°` normalizes to `0°`; no display rounding participates in classification.
