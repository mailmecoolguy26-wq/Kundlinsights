...
const { CareerReadingContextBuilder } = require('../../src/application/career-events');
...
test('title and notes cannot affect factual evidence selection', async () => {
  const a = fixture({ events: [event('a', 'DAY', 'A', 'first'), event('b', 'DAY', 'B', 'second')] }); const b = fixture({ events: [event('a', 'DAY', 'Different', 'changed'), event('b', 'DAY', 'Other', 'changed')] }); const left = await a.builder.build({ principal: {}, birthProfileId: 'profile-a' }), right = await b.builder.build({ principal: {}, birthProfileId: 'profile-a' }); assert.deepEqual(left.historicalEvidence, right.historicalEvidence); assert.deepEqual(left.futureOccurrences, right.futureOccurrences); assert.notDeepEqual(left.eventReferences, right.eventReferences);
});
test('selects at most twelve historical patterns with category diversity then factual priorities', async () => {
  const patterns = Array.from({ length: 15 }, (_, i) => pattern(`p${i}`, i < 2 ? ['A', 'B'][i] : 'A', i === 0 ? 2 : 3, i === 0 ? 1 : .5)); const x = fixture({ events: [event('a'), event('b')], patterns }); const out = await x.builder.build({ principal: {}, birthProfileId: 'profile-a' }); assert.equal(out.historicalEvidence.length, 12); assert.deepEqual(out.historicalEvidence.slice(0, 2).map((item) => item.category).sort(), ['A', 'B']); assert.equal(out.historicalEvidence.every((item) => item.matchedEventCount === 3), true); assert.equal(out.historicalEvidence.some((item) => item.patternKey === 'p0'), false);
});
test('limits future evidence across composites then exact chronological occurrences', async () => {
  const composites = Array.from({ length: 2 }, (_, i) => ({ from: `2025-01-0${i + 1}T00:00:00.000Z`, to: `2025-01-0${i + 2}T00:00:00.000Z`, matchedPatternKeys: [`c${i}`], matchedPatternCount: 2, matchedPrimaryPatternCount: 1 })); const matches = Array.from({ length: 15 }, (_, i) => ({ patternKey: `m${i}`, matchType: 'EXACT', occurrenceType: 'EVENT', instant: `2025-02-${String(i + 1).padStart(2, '0')}T00:00:00.000Z`, dimensions: [], historicalEvidence: { matchedEventCount: 2, eligibleEventCount: 2, recurrenceRate: 1 }, provenance: { calculationStatus: 'PROVISIONAL' } })); const x = fixture({ events: [event('a'), event('b')], composites, matches }); const out = await x.builder.build({ principal: {}, birthProfileId: 'profile-a' }); assert.equal(out.composites.length, 2); assert.equal(out.futureOccurrences.length, 10); assert.equal(out.futureOccurrences[0].patternKey, 'm0'); assert.equal(out.futureOccurrences.at(-1).patternKey, 'm9');
});
test('keeps a calibrated no-pattern result factual and preserves compact composites', async () => {
