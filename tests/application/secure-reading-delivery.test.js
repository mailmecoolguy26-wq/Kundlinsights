...
test('Career Insight delivery exposes a safe structured trace without internal evidence identifiers', async () => {
  ...
  assert.deepEqual(detail.insights[0].evidenceTrace.signals[0].ashtakavarga, [{ sourceFamily: 'ASHTAKAVARGA', scoreType: 'SAV', houseNumber: 10, rashiIndex: 9, value: 29 }]);
  assert.deepEqual(detail.insights[0].evidenceTrace.signals[0].planetaryState, [{ sourceFamily: 'PLANETARY_STATE', planet: 'Saturn', chart: 'D1', state: 'RETROGRADE' }]);
  assert.deepEqual(detail.insights[0].evidenceTrace.signals[0].planetaryRelationships, [{ sourceFamily: 'PLANETARY_RELATIONSHIP', subjectPlanet: 'Saturn', targetPlanet: 'Mercury', relationshipType: 'PANCHADHA', relationship: 'neutral', chart: 'D1' }]);
  ...
  assert.equal(JSON.stringify(detail.insights).match(/evidence:private|signal:private|fact:private|privateBlob/), null);
});
