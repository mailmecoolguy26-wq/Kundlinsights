...
test('Phase16E passes the optional safe Career evidence synthesis through GET', async () => {
  const calls = { list: [], detail: [], replay: 0 }; const api = buildApi(calls);
  const response = await api.inject(request('a', '/v1/readings/reading-synthesis'));
  assert.equal(response.statusCode, 200);
  assert.deepEqual(response.json().reading.careerEvidenceSynthesis, { foundation: { family: 'CAREER_FOUNDATION' }, timing: { activeDasha: true, currentTransit: false, concurrent: false, limited: true }, corroboration: { h10: { house: 10, sav: 31 } }, calibration: { calibrationLevel: 'LIMITED', eventCount: 1 } });
