...
test('declares the narrow storage-agnostic repository contracts', () => {
  assert.deepEqual(Object.keys(REPOSITORY_CONTRACTS), ['UserRepository', 'BirthProfileRepository', 'ReadingRepository', 'EntitlementRepository', 'PaymentRepository', 'PurchaseRepository', 'SubscriptionRepository', 'ProfileEntitlementRepository', 'PaymentEventRepository', 'ProviderPaymentOrderRepository', 'CareerEventObservationRepository']);
  ...
  assert.equal(REPOSITORY_CONTRACTS.CareerEventObservationRepository.includes('replaceForEvent'), true);
